...
When you navigate to the **Infrastructure** tab in the UI for an application that has the Kubernetes provider configured, you no longer see actions, such as **Create** or **Delete**, for the Kubernetes infrastructure, including Clusters, Load Balancers, and Firewalls.

Instead of being available by default, these actions are now controlled by the following property in `settings-local.yml`:
